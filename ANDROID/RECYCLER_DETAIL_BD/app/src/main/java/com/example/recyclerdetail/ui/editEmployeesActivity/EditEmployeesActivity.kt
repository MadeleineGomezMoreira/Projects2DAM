...
class EditEmployeesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityEditEmployeesBinding.inflate(layoutInflater).apply {
            ...
            var employee = Employee(ScreenConstants.ZERO_STRING, ScreenConstants.ZERO_STRING, ScreenConstants.ZERO_STRING, 0, false, 0)
            val view = root
            setContentView(view)

            intent.extras?.let {
                val employeeId = it.getString(ScreenConstants.ID_STRING)
                viewModel.getEmployeeById(employeeId.toString())
            }

            fun getEmployeeFromScreen(): Employee {
                val gender = if (radioBtnF.isChecked) ScreenConstants.FEMALE else ScreenConstants.MALE
                return Employee(
